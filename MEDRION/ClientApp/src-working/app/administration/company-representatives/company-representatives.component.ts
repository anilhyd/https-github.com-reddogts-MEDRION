...
import { CompanyRepresentativesViewModalComponent } from './company-representatives-view-modal/company-representatives-view-modal.component';
...
export class CompanyRepresentativesComponent implements OnInit {
  ...
  ngOnInit() {
    this.criteriaService.getCompanyRepresentativesCriteria().subscribe(criteria => {
      ...
      this.summaryService.getCompanyRepresentativesSummary().subscribe(summary=>{
  ...
  onRowDoubleClicked(event) {
    this.modalService.show(CompanyRepresentativesViewModalComponent, {
      backdrop: true,
      keyboard: false,
  ...
  add() {
    let modalRef = this['self'].modalService.show(CompanyRepresentativesViewModalComponent, {
      backdrop: true,
      keyboard: false,
      ignoreBackdropClick: true,
      class:'modal-vlg modal-full-height modal-right',
