...
export class ProductComponent implements OnInit {
  ...
  ngOnInit() {
    this.criteriaService.getProductCriteria().subscribe(criteria => {
      ...
      this.summaryService.getProductSummary().subscribe(summary=>{
  ...
  onRowDoubleClicked(event) {
    this.modalService.show(ProductViewModalComponent, {
      backdrop: true,
      keyboard: false,
  ...
  add() {
    let modalRef = this['self'].modalService.show(ProductViewModalComponent, {
      backdrop: true,
      keyboard: false,
      ignoreBackdropClick: true,
